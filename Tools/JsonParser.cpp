...
namespace Tools {
    ...
    nlohmann::json JsonParser::parseFromFile(const std::string& file_path) {
        ...
        try {
            ...
            FileManager::Logger::error("JsonParser file read failure: " + file_path + " - " + std::string(e.what()));
            throw FileReadException(file_path + ": " + std::string(e.what()));
        } catch (const std::exception& e) {
            FileManager::Logger::error("Unexpected error in JsonParser::parseFromFile: " + file_path + " - " + std::string(e.what()));
            throw FileReadException("Unexpected error while reading file: " + std::string(e.what()));
        }
    }
    ...
    nlohmann::json JsonParser::parseFromString(const std::string& json_string) {
        try {
            ...
            FileManager::Logger::error("JsonParser parse error: " + std::string(e.what()));
            throw ParseException(e.what());
        } catch (const std::exception& e) {
            FileManager::Logger::error("Unexpected error in JsonParser::parseFromString: " + std::string(e.what()));
            throw ParseException("Unexpected error: " + std::string(e.what()));
        }
    }
    ...
    void JsonParser::saveToFile(const nlohmann::json& json_data, const std::string& file_path, int indent) {
        try {
            ...
            FileManager::Logger::error("JsonParser file write failure: " + file_path + " - " + std::string(e.what()));
            throw FileWriteException(file_path + ": " + std::string(e.what()));
        } catch (const std::exception& e) {
            FileManager::Logger::error("Unexpected error in JsonParser::saveToFile: " + file_path + " - " + std::string(e.what()));
            throw FileWriteException("Unexpected error while writing file: " + std::string(e.what()));
        }
    }
    ...
    bool JsonParser::isValidJson(const std::string& json_string) {
        try {
            auto result = nlohmann::json::parse(json_string);  // Parse to validate, result unused
            (void)result;  // Explicitly mark as unused
            return true;
        } catch (...) {
            return false;
        }
    ...
    bool JsonParser::isValidJsonFile(const std::string& file_path) {
        try {
            ...
            // 尝试解析
            auto result = nlohmann::json::parse(buffer.str());  // Parse to validate, result unused
            (void)result;  // Explicitly mark as unused
            return true;
        } catch (...) {
            return false;
