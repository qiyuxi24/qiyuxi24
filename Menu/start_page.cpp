...
void start_page(sf::RenderWindow& window, sf::Cursor& handCursor, sf::Cursor& arrowCursor) {
    ...
    sf::Font font;
    try {
        font = FileManager::Menu::loadDefaultFont();
    } catch (const std::exception& e) {
        // 字体加载失败，使用默认字体（可能无法显示中文）
        FileManager::Logger::error("Failed to load default font in start_page: " + std::string(e.what()));
    }
    ...
    // 从成品根目录加载图片（通过 product_path 解析）
    if (FileManager::Menu::loadImageWithPath(startTexture, "start_screen.png")) {
    ...
    sf::Font titleFont;
    try {
        titleFont = FileManager::Menu::loadTitleFont();
    } catch (const std::exception& e) {
        // 如果所有字体都加载失败，尝试使用默认字体
        FileManager::Logger::error("Failed to load title font in start_page: " + std::string(e.what()));
        try {
            titleFont = FileManager::Menu::loadDefaultFont();
        } catch (const std::exception& e2) {
            // 字体加载完全失败
            FileManager::Logger::error("Failed to load default font as fallback in start_page: " + std::string(e2.what()));
        }
    }
    ...
    try {
        subtitleFont = FileManager::Menu::loadSubtitleFont();
    } catch (const std::exception& e) {
        // 如果仿宋字体加载失败，尝试使用默认字体
        FileManager::Logger::error("Failed to load subtitle font in start_page: " + std::string(e.what()));
        try {
            subtitleFont = FileManager::Menu::loadDefaultFont();
        } catch (const std::exception& e2) {
            // 字体加载完全失败
            FileManager::Logger::error("Failed to load default font as fallback for subtitle in start_page: " + std::string(e2.what()));
        }
    }
    ...
    if (buttonClicked) {
        ...
        sf::Font songFont;
        try {
            songFont = FileManager::Menu::loadDefaultFont();
        } catch (const std::exception& e) {
            // 字体加载失败
            FileManager::Logger::error("Failed to load default font for song selection in start_page: " + std::string(e.what()));
        }
